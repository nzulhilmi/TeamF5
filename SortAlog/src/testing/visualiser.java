...
public class visualiser extends JPanel {
	...
	public visualiser(algModel model, int x, int y, ArrayList<int[]> sorted, int currentIndex) {
		super();
		this.model = model;
		this.sorted = sorted;
		...
		/*
		 * for(int i = 0; i < numBoxes; i++) { if(i > 0) { xCoord = xCoord +
		 * boxSize + (boxSize/5); } boxes[i] = new box(xCoord, yCoord, boxSize,
		 * String.valueOf(currentList[i])); //haneg content values }
		 */
	}
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < numBoxes; i++) {
			/*
			 * if(i > 0) { xCur = xCur + boxSize + (boxSize/5); }
			 */
			xCur = xCur + boxSize + (boxSize / 5);
			boxes[i] = new box(xCur, yCoord, boxSize, String.valueOf(getCurrentList()[i])); // haneg
																							// content
																							// values
		}
		...
		// System.out.println(getCurrentList()[0] +" "+ getCurrentList()[1] +"
		// "+ getCurrentList()[2]);
	}
	/*
	 * public int getIndexAtPosition(int n){ return sorted.get(currentIndex); }
	 */
